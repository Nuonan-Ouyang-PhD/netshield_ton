...
% ============================================================
\begin{abstract}
Multi-probe edge security systems generate network demand endogenously:
a runtime action determines how much evidence is offered to a shared
transport-and-core service. We develop \NetShield{} as a feasibility
framework for this coupling. For $n$ probes, let $m$ retain remote evidence
in a service round, of which $k$ send full evidence of size $B$ and the
remaining $m-k$ send summaries of size $S<B$. One-round byte feasibility is
exactly $kB+(m-k)S\leq C$. If non-selected probes may send no remote
evidence, the maximum one-round full-evidence count is
$K_{\rm full}^{\max}=\min\{n,\lfloor C/B\rfloor\}$; requiring every probe
to remain remotely represented reduces this to a summary-preserving
boundary. For the three-probe testbed with $B=128$~KiB and $S=16$~KiB,
universal per-round coverage costs one full-evidence slot at both low and
mid capacity, but none at high capacity. We place this result inside a
hierarchy of same-round, finite-deadline, and long-run load feasibility.

The frozen independent queue-penalized controller exhibits a
symmetry-induced full/none limit cycle. Under a reduced saturated regime,
flow conservation gives a long-run full-request fraction
$p^\star=\min(1,C/(nB))$; a 20-seed phase-randomized software study matches
$1/3$, $2/3$, and $1$ at the three capacities. A separately frozen 36-arm
cyclic rotating full-admission (\CRFA{}) hardware extension then tests the
unrestricted frontier directly. Its post-release timely full-core output is
0.3056, 0.6327, and 0.9660 per generated event at low, mid, and high
capacity, corresponding to 91.7\%, 94.9\%, and 96.6\% of the byte frontier.
At mid capacity, relaxing universal per-round coverage raises timely
full-core output from 0.2938 under coordinated summary-preserving admission
to 0.6327 under \CRFA{} (paired difference $+0.3389$, 95\% session-bootstrap
interval $[0.3306,0.3478]$). At high capacity, where the coverage constraint
is non-binding, the paired difference is $-0.0037$
$[-0.0133,0.0046]$. Every selected \CRFA{} full result is eventually
returned, so the remaining gap to the byte frontier is a physical timing
residual rather than loss of evidence.

The primary 216-arm campaign and the 36-arm extension together account for
252 accepted hardware arms and 68,040 measured events. Their independently
reconstructed service traces contain 74,340 rounds with zero shared
application-byte cap violations. The resulting contribution is a networking
characterization of the price of per-round evidence coverage, controller
synchronization, and the gap between byte feasibility and physical
deadline attainment.
\end{abstract}
...
The formal controller artifacts also expose a second networking issue:
synchronization. The frozen independent queue-aware selector uses
\begin{equation}
\psi_i(a,t)=v u(a)-Q_i(t)b(a)/10^6,
\label{eq:qpg}
\end{equation}
where $v$ weights the frozen utility $u(a)$ against the queue--payload
penalty, $Q_i(t)$ is the local evidence queue in bytes, and $b(a)$ is the
action payload in bytes. When identical probes decide at synchronized epochs, equal queue
states feed back into equal actions. At mid capacity, this produces a
period-two full/none orbit. A poor synchronized result therefore cannot be
interpreted as evidence that decentralization itself is inferior.

The paper instead asks four questions:
...
First, the exact coverage-conditioned region and unrestricted frontier
$K_{\rm full}^{\max}=\min\{n,\lfloor C/B\rfloor\}$ quantify the
\emph{price of universal per-round remote coverage}. A cyclic construction
equalizes frontier-attaining full-evidence opportunities over time.

Second, a three-layer characterization separates same-round admission,
finite-$D$ deadline feasibility, and long-run load admissibility, distinguishing
instantaneous constraints from deadline service and average-rate conditions.

Third, the analysis explains the synchronized independent limit cycle and
gives a conditional rate-balance law for the desynchronized
\Hfull/\Hnone{} regime. Phase-randomized software conforms to its predicted
request fractions, isolating synchronization from decentralization.

Fourth, three heterogeneous physical Raspberry Pi probes test both boundaries.
A 216-arm primary campaign characterizes coordinated and independent control;
a separately frozen 36-arm \CRFA{} extension instantiates the unrestricted
1/2/3-full operating points. It attains 91.7\%, 94.9\%, and 96.6\% of the
low/mid/high byte frontier under the 1-s post-release SLO. Auditable hardware
timestamps separate byte feasibility from orchestration and wireless/core
timing residuals in both campaigns.

\paragraph{Claim boundary.}
\NetShield{} does not claim a new intrusion classifier, state-of-the-art
CICIoT2023 accuracy~\cite{neto2023ciciot}, global stochastic-control
optimality, or empirical superiority of the safety filter. The primary claims
concern network-byte feasibility, the cost of per-round coverage, controller
synchronization, and service timing.
...
requests can be sustained jointly. The distinction is important because
closed-set replay accuracy does not by itself establish deployment
performance outside the training/test world~\cite{sommer2010closedworld}.
...
parekh1993gps}. \NetShield{} adds an upstream decision in which the evidence
action itself determines offered load.
...
The frozen selector studied here is deliberately narrower: it is the finite
action score in~\eqref{eq:qpg}, not a claimed implementation of a complete
stochastic Lyapunov optimizer. We therefore use the descriptive names
independent queue-penalized greedy (\IQPG), coordinated QPG (\CQPG), and
safety-filtered coordinated QPG (\SCQPG).
...
~\cite{shisher2024timely,ari2026goal}. \NetShield{} controls an upstream
security decision: the action determines the quantity and richness of
evidence offered to the network before the transport/core service executes.
...
that architecture as an optional local-feasibility interface, but the present
campaign does not show an independent performance advantage over ordinary
capacity admission.
...
Consider $n$ edge probes sharing an application-level service budget $C(t)$
bytes in coordination round $t$. Probe $i$ has evidence queue $Q_i(t)$ and
...
\label{eq:queue}
\end{equation}
The low, mid, and high labels used in the hardware campaign denote controlled
application-byte budgets per coordination round, not measured Wi-Fi PHY
throughput. Throughout, MB denotes $10^6$ bytes, while KiB and MiB denote
...
The one-round region is the strongest of three useful service notions. Let
...
demand criterion for a preemptive single fluid server requires that, for any
...
The frozen independent policy uses~\eqref{eq:qpg}. At $v=1$, a positive-
payload action $a$ can beat \Hnone{} only while
\begin{equation}
...
The proposition is conditional: it characterizes a saturated reduced regime
rather than claiming that every decentralized policy must attain $\rho$.
...
The filter is not part of the network byte theorems. In the hardware campaign,
low-capacity safety filtering and ordinary shared-capacity admission reach the
same final summary mix, so no independent performance advantage is claimed.
...
\end{tabular}
\end{table}
These budgets regulate application payload service and are not measurements
of 1/2/3-Mbit/s physical Wi-Fi links.
...
\small
\input{tables/full_evidence_frontier.tex}
\end{table}
...
\caption{Frozen $v=1$ mid-capacity I-QPG queue trajectory and period-two
action state.}
\label{fig:cycle}
\end{figure*}
...
\caption{Frozen-selector sensitivity across the 12 tested queue weights.}
\label{fig:vsweep}
\end{figure*}
...
exchange under the tested workload, not a causal A/B effect.
...
visibility for richer evidence over time; neither operating point is
...
generated events, matching the fraction admitted by \CRFA{}. Yet its software
timely-full output is 0.3352, versus 0.6327 for \CRFA{} on hardware. This is
an interpretive contrast, not a direct method comparison across matched
physical conditions. It shows why full-request rate alone cannot determine
deadline-level service: independent queue dynamics can use the admissible
long-run rate while retaining deadline misses.
...
synchronization are distinct mechanisms, not a centralized-versus-decentralized
ranking.
...
Choose the coverage requirement before the controller. If every probe must
retain remote evidence each round, reserve the summary baseline ($C\geq nS$)
and admit full upgrades within the remaining budget. If intermittent remote
coverage is acceptable, \CRFA{} exchanges per-round universality for
full-evidence opportunities with measured maximum inter-selection intervals
of 3/2/1 rounds at low/mid/high capacity. These are admission-opportunity
bounds, not guarantees on result-return time or evidence age.

Supplementary Section~IV derives two planning extensions: a binomial-quantile
budget for a tolerated probability of any full-to-summary downgrade under
independent within-round requests, and a summary-baseline/upgrade-cost
formulation for heterogeneous payloads. They are analytical planning models,
not additional hardware findings or models fitted to QPG request traces.

Without a coordinator, phase offsets remain a candidate mitigation for
lockstep decisions, not a guarantee of byte-frontier attainment. For
schedule-inclusive deadlines, evaluate absolute-clock release scheduling
through aligned reruns of both campaigns; the post-release SLO alone does
not certify this stricter requirement.
...
additional full evidence may increase deadline pressure. The framework does
not map evidence bytes to incident-resolution utility or establish a
security-optimal policy. Operators must choose the coverage--richness
requirement within the measured service-feasibility boundary.
...
not measure Wi-Fi timing for a physical phase-randomized deployment.

\textbf{Application budget versus physical link rate.}
Low/mid/high are application-byte service budgets per coordination round, not
measured PHY throughput.
...
$[0.3306,0.3478]$. This is a coverage--richness trade-off, not universal
controller dominance. Synchronized queue feedback creates a separate
